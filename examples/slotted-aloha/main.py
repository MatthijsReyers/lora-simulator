...
SLOT_DURATION = 0.1 + estimate_airtime(
...
# Probability of transmitting in any given slot (per node)
TX_PROB = G / NUM_NODES
    ...
    async def run(self):
        ...
        try:
            ...
            while sim.is_running():
                current_slot += 1
                await sim.sleep_until(current_slot * SLOT_DURATION)
                # Slotted ALOHA: decide whether to transmit this slot
                if random.random() < TX_PROB:
                    global tx_attempts
                    tx_attempts += 1
...
if __name__ == "__main__":
    ...
    G_measured = tx_attempts * SLOT_DURATION / SIM_DURATION
    S = successful_rx * SLOT_DURATION / SIM_DURATION
